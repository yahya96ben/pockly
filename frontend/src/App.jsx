...
/* Build the customer-side message that opens in their WhatsApp */
const buildCustomerOrderMessage = (order, restaurant) => {
  const orderCode = order?.shortId || order?.id?.slice(0, 6)?.toUpperCase() || order?.id || "";
  const lines = [];
  lines.push(`Bonjour ${restaurant?.name || "le commerce"},`);
  lines.push("");
  lines.push(`Je confirme ma commande #${orderCode} :`);
  lines.push("");
  for (const item of order.items) {
    const detail = item.details ? ` (${item.details})` : "";
    lines.push(`• ${item.qty}× ${item.name}${detail}`);
  }
  lines.push("");
  lines.push(`Total : ${new Intl.NumberFormat("fr-BE",{style:"currency",currency:"EUR"}).format(order.total)}`);
  if (order.pickupTime) lines.push(`Retrait souhaité : ${order.pickupTime}`);
  if (order.paymentMethod === "place") lines.push("Paiement : sur place");
  if (order.paymentMethod === "online") lines.push("Paiement : en ligne (à confirmer)");
  if (order.remarks) lines.push(`Note : ${order.remarks}`);
  lines.push("");
  lines.push(`— ${order.customerName || "Un client"}`);
  return lines.join("\n");
};
...
function printReceipt(order, shopName) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Ticket</title><style>*{margin:0;font-family:'Courier New',monospace;font-size:13px}body{padding:20px;max-width:300px;margin:auto}</style></head><body>
    ...
    <div style="text-align:center;margin-top:20px;font-size:11px;color:#999">Pockly by Nexora</div>
  ...
  const ConfirmEl = orderConfirm ? (
    <div style={S.overlay} onClick={() => setOrderConfirm(null)}>
      <div style={S.modal} onClick={e => e.stopPropagation()}>
        ...
        {(() => {
          const restaurantPhone = currentClientShop?.phone;
          const waUrl = restaurantPhone ? buildWaUrl(restaurantPhone, buildCustomerOrderMessage(orderConfirm, currentClientShop)) : null;
          return waUrl ? (
            <div style={{marginTop:14,padding:"12px 14px",background:"#f0fdf4",borderRadius:12,border:"1px solid #bbf7d0",textAlign:"left"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                ...
                <span style={{fontSize:13,fontWeight:600,color:"#166534"}}>Étape suivante : confirmer via WhatsApp</span>
              </div>
              <p style={{fontSize:12,color:"#15803d",margin:"0 0 10px",lineHeight:1.5}}>Cliquez ci-dessous pour envoyer votre commande directement au commerçant via WhatsApp.</p>
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,width:"100%",padding:"12px 0",background:C.waGreen,color:"#fff",borderRadius:10,fontSize:14,fontWeight:600,textDecoration:"none",fontFamily:F.body}}>
                <I d={IC.wa} s={18} c="#fff" f="#fff"/> Ouvrir WhatsApp
              </a>
            </div>
          ) : null;
        })()}
      ...
      <div style={{minHeight:"100vh",background:C.bg,fontFamily:F.body,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32}}>
        <div style={{textAlign:"center",marginBottom:40,animation:"fadeUp .5s ease"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:10,marginBottom:16}}>
            <div style={{width:44,height:44,borderRadius:14,background:`linear-gradient(135deg,${C.accent},${C.accentH})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:22,fontWeight:800}}>N</div>
            <span style={{fontSize:28,fontWeight:800,color:C.text}}>Nexora</span>
          </div>
...
/* ═══════════════════════════════════════════════════════════
   CART VIEW — with checkout (name + phone + remarks + pickup + payment)
   ═══════════════════════════════════════════════════════════ */
function CartView({ cart, total, count, shop, onRemove, onBack, onPlaceOrder }) {
  ...
  const [payMethod, setPayMethod] = useState("place");
  ...
  const doConfirm = async () => {
    ...
    setProcessing(true);
    const pt = pickupMode === "asap" ? `~${shop.prepTime || 15} min` : pickupTime;
    await sleep(payMethod === "online" ? 1500 : 400);
    ...
    <div style={{maxWidth:520,margin:"0 auto",padding:"20px 16px 40px",animation:"fadeUp .3s ease"}}>
      ...
      <div className="checkout-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <label style={{display:"block"}}>
          <span style={{fontSize:13,fontWeight:600,color:C.text}}>Votre prénom *</span>
          <input style={{...S.inp,marginTop:4}} value={name} onChange={e => setName(e.target.value)} placeholder="Jean"/>
        </label>
        <div>
      ...
      <label style={{display:"block",marginBottom:14}}>
        <span style={{fontSize:13,fontWeight:600,color:C.text,display:"flex",alignItems:"center",gap:4}}>Remarques <span style={{fontWeight:400,color:C.muted}}>(optionnel)</span></span>
        <textarea style={{...S.inp,minHeight:50,resize:"vertical",marginTop:4}} value={remarks} onChange={e => setRemarks(e.target.value)} placeholder="Allergies, sans sauce, etc."/>
      </label>

      {/* Pickup */}
      <div style={{marginBottom:14}}>
        <span style={{fontSize:13,fontWeight:600,color:C.text,display:"flex",alignItems:"center",gap:4,marginBottom:6}}><I d={IC.clock} s={15} c={C.accent}/> Retrait</span>
        <div style={{display:"flex",gap:8}}>
          <button onClick={() => setPickupMode("asap")} style={{...S.choiceBtn,flex:1,...(pickupMode==="asap"?S.choiceSel:{})}}>
            <span style={{fontSize:18}}>⚡</span>
            <strong style={{fontSize:13}}>Dès que prêt</strong>
            <span style={{fontSize:12,color:C.muted}}>~{shop.prepTime || 15} min</span>
          </button>
          <button onClick={() => setPickupMode("later")} style={{...S.choiceBtn,flex:1,...(pickupMode==="later"?S.choiceSel:{})}}>
            <span style={{fontSize:18}}>🕐</span>
            <strong style={{fontSize:13}}>Plus tard</strong>
            {pickupMode === "later" && <input type="time" value={pickupTime} onChange={e => setPickupTime(e.target.value)} style={{...S.inp,padding:"3px 6px",fontSize:13,marginTop:2,textAlign:"center"}} onClick={e => e.stopPropagation()}/>}
          </button>
        </div>
      </div>
      ...
      <div style={{marginBottom:20}}>
        <span style={{fontSize:13,fontWeight:600,color:C.text,display:"flex",alignItems:"center",gap:4,marginBottom:6}}><I d={IC.creditCard} s={15} c={C.accent}/> Paiement</span>
        <div style={{display:"flex",gap:8}}>
          <button onClick={() => setPayMethod("place")} style={{...S.choiceBtn,flex:1,...(payMethod==="place"?S.choiceSel:{})}}>
            <span style={{fontSize:18}}>💰</span>
            <strong style={{fontSize:13}}>Sur place</strong>
          </button>
          <button onClick={() => setPayMethod("online")} style={{...S.choiceBtn,flex:1,...(payMethod==="online"?S.choiceSel:{})}}>
            <span style={{fontSize:18}}>💳</span>
            <strong style={{fontSize:13}}>Bancontact</strong>
          </button>
        </div>
      </div>
      ...
      <button style={{...S.btnP,width:"100%",justifyContent:"center",padding:"14px 0",fontSize:16,opacity:(name.trim()&&!phoneErr)?1:.5}} onClick={doConfirm} disabled={processing || !name.trim() || phoneErr}>
        {processing ? <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:20,height:20,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .7s linear infinite"}}/>{payMethod==="online"?"Paiement...":"Confirmation..."}</div>
        : `${payMethod==="online"?"Payer":"Confirmer"} · ${fmt(total)}`}
      </button>
